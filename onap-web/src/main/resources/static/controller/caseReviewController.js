angular
.module('app')
.controller("caseReviewController", ['$scope', '$http', '$window', '$rootScope', '$location', 'LoggingService',
                                     'Constants', 'SharedDataService', 'CaseUpdateService','FlashService', 'CommonFormService', caseReviewController]);

	function caseReviewController($scope, $http, $window, $rootScope, $location, LoggingService, Constants, SharedDataService, CaseUpdateService, FlashService,CommonFormService) {
		...
		CaseUpdateService.getLoanOfficers()
		.then(function(response) {
			if (response.success && response.data.status == "SUCCESS") {
				$scope.loanOfficer = response.data.personsList;	
			}
			...
			FlashService.error(Constants.ERROR_MESSAGES.LOAN_OFFICERS_RETRIEVAL_ERROR);
		})
		...
		$scope.LenderValidation1 = function () {
			...
			if ($scope.lender.loanOficrId != '' && 
					$scope.lender.loanOficrId != null && 
					$scope.lender.loanOficrId != undefined) { }
			else {
